...
{
    EV << "Received packet: ";
    printPacket(check_and_cast<cPacket*>(msg));
...
{
    IPAddress network(par("net").stringValue());
    IPAddress netmask(par("mask").stringValue());
    IPAddress gateway(par("gateway").stringValue());
    IPAddress begin(par("ip_begin").stringValue());
