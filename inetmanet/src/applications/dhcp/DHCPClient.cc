    ...
    if (msg->getOp()==BOOTREPLY && msg->getXid() == this->xid)
    {
        ...
        if (msg->getOptions().get(DHCP_MSG_TYPE) == DHCPOFFER && this->client_state == SELECTING)
        {
            ...
            if (!msg->getYiaddr().isUnspecified())
            {
                ...
                {
                    server_id = IPAddress(server_id_b.stringValue().c_str());
                }
        ...
        if (msg->getOptions().get(DHCP_MSG_TYPE) == DHCPACK)
        {
            ...
            if (!msg->getYiaddr().isUnspecified())
            {
                ...
                {
                    this->lease->netmask = IPAddress(netmask_b.stringValue().c_str());
                }

                if (gateway_b.intValue()>0)
                {
                    this->lease->gateway = IPAddress(gateway_b.stringValue().c_str());
                }
                if (dns_b.intValue()>0)
                {
                    this->lease->dns = IPAddress(dns_b.stringValue().c_str());
                }
                if (ntp_b.intValue()>0)
                {
                    this->lease->ntp = IPAddress(ntp_b.stringValue().c_str());
                }
