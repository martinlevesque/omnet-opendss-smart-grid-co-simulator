...
{
	{
	    ...
	    if (phyOpMode=='g')
	    {
	        ...
	        duration=SIMTIME_DBL(WifyModulationType::calculateTxDuration (btSize,modeBody, wifiPreamble));
	    }
	    else if (phyOpMode=='b')
	    {
	        // The physical layer header is sent with 1Mbit/s and the rest with the frame's bitrate
...
double Ieee80211NewRadioModel::getTestFrameError(double snirMin, double bitrate)
{
    ...
    if (fileBer)
        ...
        MpduNoError = yansModel.GetChunkSuccessRate(modeHeader,snirMin,btSize);
