...
Register_Enum(Ieee80211eMac,
              (Ieee80211eMac::IDLE,
               Ieee80211eMac::DEFER,
               Ieee80211eMac::WAITAIFS,
               Ieee80211eMac::BACKOFF,
               Ieee80211eMac::WAITACK,
               Ieee80211eMac::WAITBROADCAST,
               Ieee80211eMac::WAITCTS,
               Ieee80211eMac::WAITSIFS,
               Ieee80211eMac::RECEIVE));
...
double Ieee80211eMac::aifsn3 = 0;
double Ieee80211eMac::txop3 = 0;

/****************************************************************
 * Construction functions.
 */

Ieee80211eMac::Ieee80211eMac()
{
    endSIFS = NULL;
    endDIFS = NULL;
    endBackoff[0] = NULL;
 ...
 */
void Ieee80211eMac::initialize(int stage)
{
    ...
    if (stage == 0)
    {
        ...
        aifsn3 = (double)AIFSN[3];
        txop3 = TXOP[3].dbl();

        EV << "wireless aifsn3 = " << aifsn3 << " tx = " << txop3 << endl;
        ...
        // state variables
        fsm.setName("Ieee80211eMac State Machine");
        mode = DCF;
        sequenceNumber = 0;
        ...
        stateVector.setEnum("Ieee80211eMac");
...
void Ieee80211eMac::registerInterface()
{
...
void Ieee80211eMac::initializeQueueModule()
{
...
void Ieee80211eMac::handleUpperMsg(cPacket *msg)
{
    if (queueModule)
    {
...
void Ieee80211eMac::handleCommand(cMessage *msg)
{
...
void Ieee80211eMac::handleLowerMsg(cPacket *msg)
{
...
void Ieee80211eMac::receiveChangeNotification(int category, const cPolymorphic *details)
{
    Enter_Method_Silent();
 ...
 */
void Ieee80211eMac::handleWithFSM(cMessage *msg)
{
    ...
    FSMA_Switch(fsm)
    {
        ...
        FSMA_State(WAITACK)
        {
            ...
            //we receive ACK, but within TXOP we send next frame
...
void Ieee80211eMac::finishReception()
{
    if (getCurrentTransmission())
    {
...
simtime_t Ieee80211eMac::getEIFS()
{
...
simtime_t Ieee80211eMac::computeBackoffPeriod(Ieee80211Frame *msg, int r)
{
    int cw;
 ...
 * Timer functions.
 */
void Ieee80211eMac::scheduleSIFSPeriod(Ieee80211Frame *frame)
{
...
void Ieee80211eMac::scheduleDIFSPeriod()
{
...
void Ieee80211eMac::cancelDIFSPeriod()
{
...
void Ieee80211eMac::rescheduleAIFSPeriod(int AccessCategory)
{
    ASSERT(1);
...
void Ieee80211eMac::cancelAIFSPeriod()
{
...
void Ieee80211eMac::scheduleDataTimeoutPeriod(Ieee80211DataOrMgmtFrame *frameToSend)
{
...
void Ieee80211eMac::scheduleBroadcastTimeoutPeriod(Ieee80211DataOrMgmtFrame *frameToSend)
{
    if (!endTimeout->isScheduled())
    {
...
void Ieee80211eMac::cancelTimeoutPeriod()
{
    EV << "cancelling timeout period\n";
    cancelEvent(endTimeout);
}

void Ieee80211eMac::scheduleCTSTimeoutPeriod()
{
    if (!endTimeout->isScheduled())
    {
...
void Ieee80211eMac::scheduleReservePeriod(Ieee80211Frame *frame)
{
...
void Ieee80211eMac::invalidateBackoffPeriod()
{
    backoffPeriod[currentAC] = -1;
}

bool Ieee80211eMac::isInvalidBackoffPeriod()
{
    return backoffPeriod[currentAC] == -1;
}

void Ieee80211eMac::generateBackoffPeriod()
{
...
void Ieee80211eMac::decreaseBackoffPeriod()
{
    // see spec 9.9.1.5
...
void Ieee80211eMac::scheduleBackoffPeriod()
{
...
void Ieee80211eMac::cancelBackoffPeriod()
{
 ...
 */
void Ieee80211eMac::sendACKFrameOnEndSIFS()
{
...
void Ieee80211eMac::sendACKFrame(Ieee80211DataOrMgmtFrame *frameToACK)
{
...
void Ieee80211eMac::sendDataFrameOnEndSIFS(Ieee80211DataOrMgmtFrame *frameToSend)
{
...
void Ieee80211eMac::sendDataFrame(Ieee80211DataOrMgmtFrame *frameToSend)
{
...
void Ieee80211eMac::sendRTSFrame(Ieee80211DataOrMgmtFrame *frameToSend)
{
    EV << "sending RTS frame\n";
    sendDown(setBasicBitrate(buildRTSFrame(frameToSend)));
}

void Ieee80211eMac::sendBroadcastFrame(Ieee80211DataOrMgmtFrame *frameToSend)
{
    EV << "sending Broadcast frame\n";
    sendDown(buildBroadcastFrame(frameToSend));
}

void Ieee80211eMac::sendCTSFrameOnEndSIFS()
{
...
void Ieee80211eMac::sendCTSFrame(Ieee80211RTSFrame *rtsFrame)
{
 ...
 */
Ieee80211DataOrMgmtFrame *Ieee80211eMac::buildDataFrame(Ieee80211DataOrMgmtFrame *frameToSend)
{
...
Ieee80211ACKFrame *Ieee80211eMac::buildACKFrame(Ieee80211DataOrMgmtFrame *frameToACK)
{
...
Ieee80211RTSFrame *Ieee80211eMac::buildRTSFrame(Ieee80211DataOrMgmtFrame *frameToSend)
{
...
Ieee80211CTSFrame *Ieee80211eMac::buildCTSFrame(Ieee80211RTSFrame *rtsFrame)
{
...
Ieee80211DataOrMgmtFrame *Ieee80211eMac::buildBroadcastFrame(Ieee80211DataOrMgmtFrame *frameToSend)
{
    Ieee80211DataOrMgmtFrame *frame = (Ieee80211DataOrMgmtFrame *)frameToSend->dup();
    frame->setDuration(0);
    return frame;
...
Ieee80211Frame *Ieee80211eMac::setBasicBitrate(Ieee80211Frame *frame)
{
 ...
 * Helper functions.
 */
void Ieee80211eMac::finishCurrentTransmission()
{
...
void Ieee80211eMac::retryCurrentTransmission()
{
...
Ieee80211DataOrMgmtFrame *Ieee80211eMac::getCurrentTransmission()
{
    return transmissionQueue[currentAC].empty() ? NULL : (Ieee80211DataOrMgmtFrame *)transmissionQueue[currentAC].front();
}

void Ieee80211eMac::sendDownPendingRadioConfigMsg()
{
    if (pendingRadioConfigMsg != NULL)
    {
...
void Ieee80211eMac::setMode(Mode mode)
{
    if (mode == PCF)
...
void Ieee80211eMac::resetStateVariables()
{
...
double Ieee80211eMac::computeFrameDuration(Ieee80211Frame *msg)
{
    return computeFrameDuration(msg->getBitLength(), bitrate);
}

double Ieee80211eMac::computeFrameDuration(int bits, double bitrate)
{
    return bits / bitrate + PHY_HEADER_LENGTH / BITRATE_HEADER;
}

void Ieee80211eMac::logState()
{
    char a[10]="";
    char b[10]="";
    char c[10]="";
...
const char *Ieee80211eMac::modeName(int mode)
{
...
bool Ieee80211eMac::transmissionQueueEmpty()
{
