...
#ifndef BASIC_IEEE80211_MGMT_AP_H
#define BASIC_IEEE80211_MGMT_AP_H
 ...
 * @author Andras Varga
 */
class INET_API BasicIeee80211MgmtAP : public Ieee80211MgmtAPBase
{
  public:

	~BasicIeee80211MgmtAP();
  ...
  protected:
    ...
    MACAddress addrTrafInterface;

    std::string wiType;

    int nbPacketsRequested;

    int aggregationTresholdBytes;
    double maximumAggregationDuration;
    double queueLimit;
  ...
  protected:
    ...
    bool routeFrame(Ieee80211DataFrame *frame);
  ...
  private:
    ...
    void removeSubPacketsWithErrors(Ieee80211Frame* frame);
    int sizeInBytes(const std::vector<EtherFrame>& frames);
    double lengthOverallQueueSize();

    std::vector<EtherFrame*> convertToEtherFrames(Ieee80211DataFrame *frame);

    void frameReceivedDest(cPacket* packet);

    int sizeOfQueueInBytes(MACAddress addr, short kin);
    double aggregationDuration(MACAddress addr, short kind);

    // Addr -> kind -> frames with aggregation time
    std::map<MACAddress, std::map<short, std::list<std::pair<double, EtherFrame*> > > > queues;
    static long cntPackets;
